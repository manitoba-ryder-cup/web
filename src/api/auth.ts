...
export const authApi = {
  ...
  refresh: () => fetch(`${BASE}/v1/refresh`, { method: 'POST' }).then(json<LoginResponse>),
  logout: () => fetch(`${BASE}/v1/logout`, { method: 'POST' }),
  me: (token: string) => fetch(`${BASE}/v1/users/me`, { headers: { Authorization: `Bearer ${token}` } }).then(json<User>),
}
