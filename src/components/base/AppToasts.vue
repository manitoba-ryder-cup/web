...
<template>
  <!-- Bottom-centre, thumb-reachable and out of the way. aria-live so screen readers
       announce confirmations; tap a toast to dismiss it early. -->
  <div class="pointer-events-none fixed inset-x-0 bottom-4 z-50 flex flex-col items-center gap-2 px-4"
    ...
    <TransitionGroup
      ...
      <button v-for="t in toasts" :key="t.id" type="button" @click="dismiss(t.id)"
