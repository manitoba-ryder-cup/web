...
<template>
  <div class="flex items-center gap-3 border-b border-mrc-line px-3 py-2 last:border-b-0">
    ...
    <button v-if="showCaptain" type="button" :disabled="busy" @click="emit('toggle-captain')"
            :aria-pressed="isCaptain" :title="isCaptain ? 'Captain — tap to clear' : 'Make captain'"
            class="flex h-7 w-7 shrink-0 items-center justify-center rounded-full border text-xs font-bold transition"
            :class="isCaptain ? 'border-mrc-gold bg-mrc-gold text-white' : 'border-mrc-line text-mrc-faint hover:border-mrc-gold hover:text-mrc-gold'">C</button>
    ...
    <div class="flex shrink-0 overflow-hidden rounded border border-mrc-line" role="group" :class="busy ? 'opacity-50' : ''">
      <button type="button" :disabled="busy" @click="toggleTeam(blueId)"
              class="w-14 py-1.5 text-sm font-semibold transition"
              :class="player.team_id === blueId ? 'bg-mrc-blue-team text-white' : 'bg-white text-mrc-muted hover:bg-mrc-blue-tint'">Blue</button>
      <button type="button" :disabled="busy" @click="toggleTeam(redId)"
              class="w-14 border-l border-mrc-line py-1.5 text-sm font-semibold transition"
              :class="player.team_id === redId ? 'bg-mrc-red-team text-white' : 'bg-white text-mrc-muted hover:bg-mrc-red-tint'">Red</button>
    </div>
  </div>
</template>
