// The five destinations the two navs offer. Both ask this same question, so the bottom bar
// and the desktop header cannot light different tabs for the screen you are on.
export type NavSection = 'home' | 'scores' | 'news' | 'teams' | 'history'
...
export function navSection(route: { path: string; query: Record<string, unknown> }): NavSection | null {
  ...
  // An article sits under the list that holds it, so reading one keeps the tab lit.
