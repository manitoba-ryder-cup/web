...
// Offered where the wrong scores are visible, and only to whoever could fix them: gated on
// the scope, so for a spectator or a scorer this page is unchanged.
const holesEntered = computed(() => match.value?.hole_results.length ?? 0)
const canReset = computed(() => auth.hasScope(SCOPE_TOURNAMENTS_WRITE) && holesEntered.value > 0)
      ...
      <template v-if="match && leftTeam && rightTeam">
        ...
        >
          ...
          <template v-if="canReset" #actions>
            <BaseMenu ref="menu" label="Match actions" align="right">
