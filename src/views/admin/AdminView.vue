...
// one being worked on.
const { data, error, loading } = useAsync(() => scorecardApi.listTournaments())
const tournaments = computed(() =>
  [...(data.value ?? [])].sort((a, b) => b.start_date.localeCompare(a.start_date)))
</script>
<template>
  <PageLayout title="Admin" image="/img/oceanside.webp">
    <AsyncState :loading="loading" :error="error">
      ...
      <div class="space-y-3">
        <LinkCard v-for="t in tournaments" :key="t.id"
                  :to="{ name: 'admin-tournament', params: { id: t.id } }">
